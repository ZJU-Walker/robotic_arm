...
void FSUS_Servo::init() {
  ...
  ping();
  if (this->isOnline) {
    // 舵机如果在线就开始同步角度 ，当不在原始角度范围会突然回到0度， 请注意安全。
    queryAngle();
    // 目标角度跟初始角度一致
...
void FSUS_Servo::calibration(FSUS_SERVO_ANGLE_T rawA, FSUS_SERVO_ANGLE_T realA,
                             FSUS_SERVO_ANGLE_T rawB,
                             FSUS_SERVO_ANGLE_T realB) {
...
// 设置舵机的角度
void FSUS_Servo::setAngleRange(FSUS_SERVO_ANGLE_T angleMin,
                               FSUS_SERVO_ANGLE_T angleMax) {
...
// 限制舵机期望原始角度范围到舵机原始角度方位内 sunny add
bool FSUS_Servo::constrainRawAngle(FSUS_SERVO_ANGLE_T& raw_angle) {
  if (raw_angle >= -FSUS_PARAM_RAW_ANGLE_LIMIT &&
      raw_angle <= FSUS_PARAM_RAW_ANGLE_LIMIT) {
    return true;
  } else {
    raw_angle = constrain(raw_angle, -FSUS_PARAM_RAW_ANGLE_LIMIT,
                         FSUS_PARAM_RAW_ANGLE_LIMIT);
    return false;
  }
}

// 设置舵机的平均转速
void FSUS_Servo::setSpeed(FSUS_SERVO_SPEED_T speed) { this->speed = speed; }
...
void FSUS_Servo::setRawAngle(FSUS_SERVO_ANGLE_T rawAngle,
                             FSUS_INTERVAL_T interval, FSUS_POWER_T power) {
  this->isMTurn = false;
  constrainRawAngle(rawAngle);//sunny add
...
// 设置舵机的原始角度(指定周期)
void FSUS_Servo::setRawAngleByInterval(FSUS_SERVO_ANGLE_T rawAngle,
                                       FSUS_INTERVAL_T interval,
                                       FSUS_INTERVAL_T t_acc,
                                       FSUS_INTERVAL_T t_dec,
                                       FSUS_POWER_T power) {
  this->isMTurn = false;
   constrainRawAngle(rawAngle);//sunny add
  this->targetRawAngle = rawAngle;
  this->protocol->sendSetAngleByInterval(this->servoId, rawAngle, interval,
                                         t_acc, t_dec, power);
}

// 设定舵机的原始角度(指定转速)
void FSUS_Servo::setRawAngleByVelocity(FSUS_SERVO_ANGLE_T rawAngle,
                                       FSUS_SERVO_SPEED_T velocity,
                                       FSUS_INTERVAL_T t_acc,
                                       FSUS_INTERVAL_T t_dec,
                                       FSUS_POWER_T power) {
  this->isMTurn = false;
  constrainRawAngle(rawAngle);//sunny add
  this->targetRawAngle = rawAngle;
  this->protocol->sendSetAngleByVelocity(this->servoId, rawAngle, velocity,
                                         t_acc, t_dec, power);
}
...
// 设定舵机的原始角度(多圈+指定周期)
void FSUS_Servo::setRawAngleMTurnByInterval(FSUS_SERVO_ANGLE_T rawAngle,
                                            FSUS_INTERVAL_T_MTURN interval,
                                            FSUS_INTERVAL_T t_acc,
                                            FSUS_INTERVAL_T t_dec,
                                            FSUS_POWER_T power) {
  this->isMTurn = true;
  this->targetRawAngle = rawAngle;
  this->protocol->sendSetAngleMTurnByInterval(this->servoId, rawAngle, interval,
                                              t_acc, t_dec, power);
}

// 设定舵机的原始角度(多圈+指定转速)
void FSUS_Servo::setRawAngleMTurnByVelocity(FSUS_SERVO_ANGLE_T rawAngle,
                                            FSUS_SERVO_SPEED_T velocity,
                                            FSUS_INTERVAL_T t_acc,
                                            FSUS_INTERVAL_T t_dec,
                                            FSUS_POWER_T power) {
  this->isMTurn = true;
  this->targetRawAngle = rawAngle;
  this->protocol->sendSetAngleMTurnByVelocity(this->servoId, rawAngle, velocity,
                                              t_acc, t_dec, power);
}
...
uint16_t FSUS_Servo::queryVoltage() {
  ...
  uint8_t content[2];
  FSUS_STATUS status =
      this->protocol->recvReadData(&servoId, &address, &contentLen, content);
...
/* 查询舵机型号 */
uint32_t FSUS_Servo::queryType() {
  // 发送数据
  this->protocol->sendReadData(this->servoId, FSUS_PARAM_TYPE);
  // 接收数据
  FSUS_SERVO_ID_T servoId;
  uint8_t address;
  uint8_t contentLen;
...
/* 查询舵机固件版本 */
uint32_t FSUS_Servo::queryFirmware() {
  // 发送数据
  this->protocol->sendReadData(this->servoId, FSUS_PARAM_FIRMWARE);
  // 接收数据
  FSUS_SERVO_ID_T servoId;
  uint8_t address;
  uint8_t contentLen;
...
/* 查询舵机的SN */
uint32_t FSUS_Servo::querySN() {
  // 发送数据
  this->protocol->sendReadData(this->servoId, FSUS_PARAM_SN);
  // 接收数据
  FSUS_SERVO_ID_T servoId;
  uint8_t address;
  uint8_t contentLen;
  ...
  if (status == FSUS_STATUS_SUCCESS) {
    // 提取数据
    int32_t sn;
    byte* valuePtr = (byte*)&sn;
    valuePtr[0] = content[0];
    valuePtr[1] = content[1];
    valuePtr[2] = content[2];
    valuePtr[3] = content[3];
    return sn;
  }
  return 0;
}
...
void FSUS_Servo::setDamping() { setDamping(500); }
...
bool FSUS_Servo::isStop() {
  ...
  return abs(this->curRawAngle - this->targetRawAngle) <=
         FSUS_ANGLE_CTL_DEADBLOCK;
}
...
void FSUS_Servo::wait() {
  ...
  while (!isStop()) {
    // 角度误差保持不变(判断条件1°)，如果未到位且角度误差不变（卡死），1s后跳出while循环；角度误差变化继续等待.
...
// 轮子停止
void FSUS_Servo::wheelStop() { this->protocol->sendWheelStop(this->servoId); }
...
void FSUS_Servo::wheelRunNCircle(uint8_t is_cw, uint16_t circle_num) {
  this->protocol->sendWheelMoveNCircle(this->servoId, is_cw, this->speed,
                                       circle_num);
}
