...
#define FSARM_STATUS_RAW1_OUTRANGE 20  // 关节1超出范围
#define FSARM_STATUS_JOINT2_OUTRANGE 3  // 关节2超出范围
#define FSARM_STATUS_RAW2_OUTRANGE 30  // 关节2超出范围
#define FSARM_STATUS_JOINT3_OUTRANGE 4  // 关节3超出范围
#define FSARM_STATUS_RAW3_OUTRANGE 40  // 关节3超出范围
#define FSARM_STATUS_JOINT4_OUTRANGE 5  // 关节4超出范围
#define FSARM_STATUS_RAW4_OUTRANGE 50  // 关节3超出范围
#define FSARM_STATUS_TOOLPOSI_TOO_FAR 6  // 工具坐标期望点距离机械臂太遥远
#define FSARM_STATUS_WRISTPOSI_TOO_FAR 7  // 腕关节坐标期望点距离机械臂太遥远
#define FSARM_STATUS_WRISTPOSI_TOO_NEAR 8  // 腕关节坐标期望点距离机械臂太遥远
...
#define FSARM_LINK3 8.118  // 连杆3的长度 单位cm
#define FSARM_LINK4 14  // 连杆4的长度 单位cm(到手爪尖端的长度)

// 舵机标定参数
#define FSARM_JOINT1_P90 -108  //-63//-90.70  //关节1为90°时的舵机原始角度
#define FSARM_JOINT1_N90 71  // 117//91.9    //关节1为-90°时的舵机原始角度
#define FSARM_JOINT2_P0 -86.7  //-95//90.80   //关节2为0°时的舵机原始角度
#define FSARM_JOINT2_N90 2.3  //-5//0.90    //关节2为-90°时的舵机原始角度
#define FSARM_JOINT3_P90 94  //-45.5   //关节3为90°时的舵机原始角度
#define FSARM_JOINT3_N90 -86  // 130.90  //关节3为-90°时的舵机原始角度
#define FSARM_JOINT4_P90 96  //-93.4   //关节4为90°时的舵机原始角度
#define FSARM_JOINT4_N90 -84  // 84.3    //关节4为-90°时的舵机原始角度

// 爪子的配置
#define GRIPPER_SERVO_ID 4             // 爪子对应的舵机ID
#define GRIPPER_SERVO_ANGLE_OPEN -100    // 15.0   // 爪子张开时的角度
#define GRIPPER_SERVO_ANGLE_CLOSE -115  //-45.0 // 爪子闭合时的角度
#define GRIPPER_INTERVAL_MS 1000       // 爪子开启闭合的周期, ms
#define GRIPPER_MAX_POWER \
  500  // 爪子的最大功率
       // 注:对于大功率的舵机需要调大这个值
...
#define FSARM_JOINT2_MIN -180.0
#define FSARM_JOINT2_MAX 0
...
// HOME(机械零点的位置)
#define FSARM_HOME_X 13.5  //单位cm
#define FSARM_HOME_Y 0
#define FSARM_HOME_Z 6.4
#define FSARM_HOME_PITCH 20.0  //单位°
//转向是否一致 sunny add

#define SAME_DIRECTION 1
#define OPPO_DIRECTION 0
...
class FSARM_ARM5DoF {
 public:
  ...
  // 关节标定简化版 sunny add
  //参数 各关节竖直安装位的角度 旋转方向
  void calibration(float joint1_P0, bool direction1, float joint2_N90,
                   bool direction2, float joint3_P0, bool direction3,
                   float joint4_P0, bool direction4);
  // 设置舵机角度范围
  void setAngleRange();
  // 开启扭矩
  ...
  // 设置单个舵机的原始角度 控制间隔 sunny add
    void setRawAngle(FSARM_JOINTS_STATE_T thetas, uint16_t interval);
  // 设置关节的角度
  ...
  void forwardKinematics(FSARM_JOINTS_STATE_T thetas, FSARM_POINT3D_T* toolPosi,
                         float* pitch);
  // 机械臂逆向运动学
  FSARM_STATUS inverseKinematics(FSARM_POINT3D_T toolPosi, float pitch,
                                 FSARM_JOINTS_STATE_T* thetas);
  FSARM_STATUS inverseKinematics(FSARM_POINT3D_T toolPosi,
                                 FSARM_JOINTS_STATE_T* thetas);
  // 腕关节转换为末端的坐标
  ...
  void gripperOpen();
  // 夹爪张开
  void gripperOpen(float angle);
  // 夹爪闭合
  void gripperClose();
  // 夹爪闭合 带参数
  void gripperClose(float angle);
